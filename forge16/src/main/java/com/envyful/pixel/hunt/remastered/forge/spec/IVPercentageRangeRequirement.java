...
public class IVPercentageRangeRequirement extends AbstractPokemonRequirement<Pair<Integer, Integer>> {
    ...
    public List<Requirement<Pokemon, PixelmonEntity, ?>> createSimple(String key, String spec) {
        ...
        return Collections.singletonList(new IVPercentageRangeRequirement(min, max));
    }

    @Override
    public Requirement<Pokemon, PixelmonEntity, Pair<Integer, Integer>> createInstance(Pair<Integer, Integer> percentage) {
        return new IVPercentageRangeRequirement(percentage.getA(), percentage.getB());
    }
